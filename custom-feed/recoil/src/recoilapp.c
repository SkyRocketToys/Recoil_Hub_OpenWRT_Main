...
Data_t gData[MAX_NETWORK_CLIENT_IDS];
...
void RECOILAPP_Setup (void)
{
	...
	gNetworkUpgradePort = NETWORK_UPGRADE_PORT_RECOILAPP;
...
bool RECOILAPP_Start (void)
{
    ...
    else
    {
        for (node = 0; node < MAX_NETWORK_CLIENT_IDS; node++)
        {
			DEBUG_INFO("RECOILAPP_Start attach node %d to JSON", node);
            gData[node].valid = false;
    ...
    DEBUG_OUT;
	DEBUG_INFO("RECOILAPP_Start(StartHost)");

    GameNetworkServerWrapper_StartHost();
	DEBUG_INFO("RECOILAPP_Start(Done)");
...
// ----------------------------------------------------------------------------
// Resets the PASSWORD of the WIFI ROUTER or BASE STATION, when the
// POWER BUTTON on the BASE STATION has been held down.
// This is called from the power helper thread when the Tritan chip requests a WiFi password reset
void RECOILAPP_ResetBaseStationPassword(void)
{
#ifdef ENABLE_PRODUCTION
//    system("uci set wireless.@wifi-iface[0].ssid=RECOIL_BS");
//    system("uci set wireless.@wifi-iface[0].key=\"recoil123\"");

...
//					if (NWDISCOVERY_GetClientInfoFromIP((uint8_t*)inet_ntoa(client.sin_addr), &info, CRITERIA_LAST_MATCH, &gClients) && info.connected)
...
#endif
                    {
                        ...
                        else
                        {
                            ...
                            RECOIL_SendError(true, tmphandle, (uint8_t)info.clientId, EVENT_ALREADY_CONNECTED, NULL, 0);
                        }
                    }
                    else
                    {
                        DEBUG_ERROR("Client(%s) requesting for a network event, without discovery complete !!!", (uint8_t*)inet_ntoa(client.sin_addr));
                        RECOIL_SendError(true, tmphandle, (uint8_t)info.clientId, DISCOVERY_NOT_CONNECTED, NULL, 0);
                    }
                }
...
Error_t NWEVENT_Attach (uint32_t clientId, NWEVENT_Receive_fn func)
{
    ...
    if (clientId < MAX_NETWORK_CLIENT_IDS)
    {
        pthread_mutex_lock(&gNoOfClients_mutex);
...
Error_t NWEVENT_Send (uint32_t clientId, uint8_t *buffer, uint32_t buffer_size, uint32_t *sent)
{
    ...
    *sent = 0;
    ...
    if ((clientId < MAX_NETWORK_CLIENT_IDS) && ((SIZE_OF_HEADER + buffer_size) <= MAX_INCOMING_BYTES))
    {
        pthread_mutex_lock(&gNoOfClients_mutex);

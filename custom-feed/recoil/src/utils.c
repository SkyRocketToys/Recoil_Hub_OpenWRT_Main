...
void UTILS_SetHeader (ProtocolHeaderV1_write *hdr, uint8_t id, uint16_t len)
{
    ...
    DEBUG_EINFO("UTILS_SetHeader pid(%d) version(%d) id(%d) len(%d)", hdr->pid, hdr->version, hdr->id, hdr->length);    
...
bool UTILS_VerifyPacketHeader (uint8_t *buffer, uint32_t size, ProtocolId exp_id, uint32_t exp_size, uint32_t *payload_size)
{
    ...
    if (!retval)
    {
        ...
        DEBUG_INFO("Incoming bytes size(%d) data[%2x %2x %2x %2x]", size, buffer[0], buffer[1], buffer[2], buffer[3]);
    }
...
#if 000 // CPM: is this required? to log so much data?
    int i, j;
    j = (size > 16) ? 16 : size;
    printf ("data(%d): ", size);
    for (i = 0; i < j; i++)

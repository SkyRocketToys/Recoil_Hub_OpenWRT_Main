                ...
                if ((INVALID_RETURN_VALUE != gClients[node].handle) && (FD_ISSET(gClients[node].handle, &fds)))
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            RECOIL_SendError(true, gClients[node].handle, (uint8_t)node, DISCOVERY_NOT_CONNECTED, NULL, 0);
                        }
                    }
                }                    
            }
...
//			if (NWDISCOVERY_GetClientInfoFromIP((uint8_t*)inet_ntoa(client.sin_addr), &info, CRITERIA_LAST_MATCH, &gClients) && info.connected)
			if (NWDISCOVERY_GetClientInfoFromIP((uint8_t*)inet_ntoa(client.sin_addr), &info, CRITERIA_FIRST_INVALID_HANDLE, &gClients, sizeof(ClientInfo_t)) && info.connected)
#else
...
#endif
            {
                ...
                if (SOCKET_INVALID_HANDLE == gClients[info.clientId].handle)
                {
                    node = info.clientId;
                    ...
                    else
                    {
                        ...
                        RECOIL_SendError(true, tmphandle, (uint8_t)node, SYNC_INTERNAL_ERROR, NULL, 0);
                    }
                }
                else
                {
                    ...
                    RECOIL_SendError(true, tmphandle, (uint8_t)info.clientId, SYNC_ALREADY_CONNECTED, NULL, 0);
                }
            }
            else
            {
                DEBUG_FATAL("Client requesting for a network sync, without discovery complete !!!");
                RECOIL_SendError(true, tmphandle, (uint8_t)INVALID_CLIENT_ID, DISCOVERY_NOT_CONNECTED, NULL, 0);
            }
        }
